...
- This behavior persists even with hydration enabled
...
4. Observe the console logs in both terminal and browser:
   ...
   - Example 3 (with hydration): Same as Example 2, but includes hydration fallback - still no loader calls on hover
...
- Routes with hydration: Should potentially prefetch data for hydration
...
- Routes with hydration enabled: Neither loader is called, same as without hydration
...
│   └── example-3.tsx   # Example with client loader + hydration
└── components/
...
- Example 3 adds hydration with `clientLoader.hydrate = true` (still no prefetch)
